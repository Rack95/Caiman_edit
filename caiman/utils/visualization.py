...
def nb_view_patches(Yr, A, C, b, f, d1, d2, YrA=None, image_neurons=None, thr=0.99, denoised_color=None, cmap='jet'):
    ...
    np.savetxt("temporal_components", C, newline="] [")
    ...
    #transform C into a pandas dataframe and save it in a text file
    
    df = pd.DataFrame(C/100)
    df.columns = df.columns+1
    df.index = df.index + 1
    
    df.to_csv('component_matrix_R.txt', header=True, index=False, sep='\t')
    
    np.savetxt('component_matrix_ML.txt', C/100, delimiter='\t')
    ...
    plot = bpl.figure(plot_width=1000, plot_height=500)
    plot.line('x', 'y', source=source, line_width=1, line_alpha=0.6)
    plot.xaxis.axis_label = "Time (frames)"
    plot.yaxis.axis_label = "Spiking activity"
    ...
    plot1 = bpl.figure(x_range=xr, y_range=yr, plot_width=400, plot_height=400)
    ...
    if Y_r.shape[0] > 1:
        bpl.show(bokeh.layouts.layout([[slider], [bokeh.layouts.column(plot1, plot)]]))
