...
def detrend_df_f(A, b, C, f, YrA=None, quantileMin=8, frames_window=500, 
    ...
    #transform F_df into a pandas dataframe and save it in a text file
    
    df2 = pd.DataFrame(F_df)
    df2.columns = df2.columns+1
    df2.index = df2.index + 1
    
    df2.to_csv('df_f_signal_R.txt', header=True, index=False, sep='\t')
    
    np.savetxt('df_f_signal_ML.txt', F_df, delimiter='\t')
